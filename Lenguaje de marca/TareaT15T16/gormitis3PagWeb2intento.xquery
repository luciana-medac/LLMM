...
declare function local:get-color($tribu as xs:string) as xs:string {
  if ($tribu = 'Tierra') then 'green'
  else if ($tribu = 'Mar') then 'blue'
  else if ($tribu = 'Volcan') then 'red'
  else if ($tribu = 'Luz') then 'yellow'
  else if ($tribu = 'Oscuridad') then 'black'
  else 'gray'
};

declare function local:add-id-and-color($gormitis as node()) as node() {
  for $gormiti at $i in $gormitis
  let $color := local:get-color($gormiti/tribu/text())
  return
...
let $gormitis := doc("gormitis3.xml")//gormiti
return
  <html>
    <head>
      <title>Gormitis Legendarios</title>
      <style>
        body&#123;font-family: Verdana; margin-top: 80px;&#125;
        table&#123;width: 850px; margin: 15px; border-collapse: collapse; box-shadow: 10px 10px 10px gray;&#125;
        td&#123;border: solid 1px black; text-align: center; padding: 5px;&#125;
        th&#123;border: solid 1px black; background-color: #404040; color: white; text-align: center; font-size: 1rem;&#125;
        tr&#123;height: 50px;&#125;
        tr:nth-child(odd)&#123;background-color: #EBF5FF;&#125;
